...
func TestRejectsUnavailableRolloutStrategy(t *testing.T) {
	scheme := runtime.NewScheme()
	if err := platformv1alpha1.AddToScheme(scheme); err != nil {
		t.Fatal(err)
	}
	service := NewWithClient(fake.NewClientBuilder().WithScheme(scheme).Build())
	deployment := validDeployment()
	deployment.Spec.Rollout = &platformv1alpha1.RolloutSpec{Strategy: "canary", CanaryWeight: 10}
	if err := service.Create(context.Background(), deployment); err == nil {
		t.Fatal("expected unavailable rollout strategy error")
	}
}
