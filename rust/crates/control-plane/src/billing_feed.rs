...
use xscope_entities::{billing_account, billing_consumer as consumer, billing_event as event};
...
fn acknowledged(row: &consumer::Model, ack: Option<&AckRequest>) -> ServiceResult<i64> {
    let Some(ack) = ack else { return Ok(row.acknowledged) };
    ...
    if ack.sequence == row.acknowledged { return Ok(row.acknowledged); }
    if ack.expected_sequence != row.acknowledged
        || ack.sequence < row.acknowledged || ack.sequence > row.delivered {
        return Err(conflict("stale ACK or acknowledging events that were not delivered"));
    }
    Ok(ack.sequence)
}
...
async fn account_id(tx: &DatabaseTransaction, project: &str) -> ServiceResult<String> {
    ...
    billing_account::Entity::find()
        .select_only().column(billing_account::Column::Id)
        .filter(billing_account::Column::ProjectId.eq(project))
        .into_tuple::<String>().one(tx).await?.ok_or(ServiceError::NotFound)
}

async fn cursor(tx: &DatabaseTransaction, account: &str, name: &str, exclusive: bool) -> ServiceResult<Option<consumer::Model>> {
    let query = consumer::Entity::find_by_id((account.to_owned(), name.to_owned()));
    if exclusive { query.lock_exclusive().one(tx).await } else { query.one(tx).await }
...
async fn page(tx: &DatabaseTransaction, row: &consumer::Model, request: &PollRequest) -> ServiceResult<Page> {
    ...
    let mut events = event::Entity::find()
        ...
        .order_by_asc(event::Column::Sequence).limit(request.limit + 1).all(tx).await?;
    let has_more = events.len() as u64 > request.limit;
    if has_more { events.pop(); }
    let delivered = events.last().map_or(row.delivered, |e| row.delivered.max(e.sequence));
    Ok(Page { events, acknowledged: ack, delivered, has_more })
}

impl Repository {
    pub async fn poll_billing_events(&self, project: &str, name: &str, request: PollRequest) -> ServiceResult<Value> {
        ...
        let row = if let Some(row) = cursor(&tx, &account, name, false).await? { row } else {
            ...
            consumer::Entity::insert(consumer::ActiveModel {
                billing_account_id: Set(account.clone()), consumer: Set(name.into()),
                acknowledged: Set(0), delivered: Set(0), updated_at: Set(Utc::now().fixed_offset()),
            }).on_conflict(OnConflict::columns([consumer::Column::BillingAccountId, consumer::Column::Consumer])
                .do_nothing().to_owned()).exec_without_returning(&tx).await?;
            cursor(&tx, &account, name, false).await?.ok_or(ServiceError::NotFound)?
        };
        ...
        let row = cursor(&tx, &account, name, true).await?.ok_or(ServiceError::NotFound)?;
    ...
    pub async fn ack_billing_events(&self, project: &str, name: &str, request: AckRequest) -> ServiceResult<consumer::Model> {
        if !identifier(name) { return Err(invalid("invalid consumer")); }
        let tx = self.db.begin().await?;
        let account = account_id(&tx, project).await?;
        let row = cursor(&tx, &account, name, false).await?.ok_or(ServiceError::NotFound)?;
...
mod tests {
    ...
    fn cumulative_ack_preserves_idempotence_and_delivery_boundary() {
        let row = consumer::Model { billing_account_id: "a".into(), consumer: "c".into(),
            acknowledged: 10, delivered: 20, updated_at: Utc::now().fixed_offset() };
        assert_eq!(acknowledged(&row, None).unwrap(), 10);
        for (expected_sequence, sequence, ok) in [(0, 10, true), (10, 20, true), (0, 20, false), (10, 9, false), (10, 21, false), (-1, 10, false)] {
            assert_eq!(acknowledged(&row, Some(&AckRequest { expected_sequence, sequence })).is_ok(), ok);
        }
    }
}
