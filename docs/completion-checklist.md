...
新增 [计费有界并发、业务快照恢复、站内告警和运维 UI](reliability-operations.md)。28 个 Bazel 测试目标通过，含 19 个浏览器用例；真实 PostgreSQL 协议回归、独立恢复演练、成员链路中断恢复、实际 Alertmanager 通知投递通过。控制面/Gateway 使用 Bazel Linux ARM64 release 镜像，业务备份与告警扩展部署在本地；账号、资金冻结、旧 PVC/对象证据保留。

本轮按用户要求跳过 GPU 和支付宝。每日业务快照不是异地/PITR，站内告警不是短信/邮件，成员断网 fixture 不是第二真实集群。新 UI 未补齐集群注册/凭据轮换等全部管理动作；跨地域容灾、长期容量/十年备份归档与其他商用验收仍未完成。下方是历史验收记录，不以其旧待办覆盖本节的新结果。

首轮部署时平台管理员与财务审核员 subject 名单均为空；后续按用户明确授权，完成指定现有账号的身份核验和平台管理员配置加载，当前平台管理员为 1，财务审核员保持 0。没有自动将其他租户 owner 升权，没有修改密码或创建审核员。本轮工作区和可达 Git 历史脱敏扫描均无发现，但不证明先前披露的凭据已失效，未执行轮换或历史重写。

## KEDA 与终止证明增量

代码已增加 [ModelScale 建议对象、自动排空缩容、容器绑定/终止证据、Envoy 二次授权和遗留请求空闲证明](safe-autoscaling.md)。本地部署和真实链路验收分别记录，不能以模拟 kubelet 的测试替代真实节点验证。正式支付/税务、资源池准入和跨地域容灾等更大规划仍不在此次完成范围。

本地验收已完成：四个 Rust 服务的 Bazel Linux ARM64 release 镜像已部署，成员 Agent 与受管 Echo 演示入口运行正常；真实 KEDA 完成 `2 → 1 → 2`，在长流期间等待排空。对独立临时 Gateway Pod 发起带 UID/RV 前置条件、短宽限期且保留 finalizer 的删除后，kubelet 终止证明、Envoy 空闲证据、自动缩容及重新开放均通过。公共 Gateway 未用于故障注入，临时 Gateway/Service 已清理。

原有推理链路的鉴权、SSE、取消传播和双分录结算回归通过；26 个 Bazel 测试目标通过，工作区脱敏扫描无发现。Jaeger 的 OOM 问题通过调整内存预算修复，CPU 请求仍为 20m，原 Badger PVC 未更换；所有平台 Deployment 就绪。业务库及旧 WAL 已备份到仓库外私有目录，原身份、账本、Redis 和五个 PVC 保留，测试产生的未知用量冻结未自动释放。旧 Git 历史的凭据风险不因此消除。
