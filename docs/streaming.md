...
One usage event is emitted in the final request logging phase, after downstream success or failure is known. The [HTTP outbox](usage-delivery.md) is volatile; there is no local persistence or replay mode:
...
| Downstream disconnected | `cancelled` | Counters preserved if known; known usage can settle; unknown usage keeps the central hold |
| Truncated stream/missing `[DONE]`, invalid event or upstream failure | `provider_error` | Not treated as successful usage |

If actual token usage is unknown, the Redis TPM reservation is retained until its quota window expires. It is **not** released as zero usage. Monetary admission, unresolved receipts, evidence reviews and loss waivers are covered by the [billing contract](billing-protocol.md).
...
# Reads the local test key from Kubernetes Secret at runtime; never prints it.
bazel run //tools:inference_cluster_smoke
```

The integration test starts Bazel-built gateway/runtime binaries on isolated TCP ports. It checks incremental delivery, a single HTTP usage receipt and propagated W3C trace context, cancellation after the first content delta, model rejection without starting inference, and non-streaming compatibility. Parser tests cover every two-part UTF-8/CRLF split, long streams, oversized and conflicting usage events.
