...
import { BillingReviews } from "./BillingWorkflows";
...
export function PendingReservations({ projectID, currency }: { projectID: string; currency: string }) {
  ...
  return <>
    ...
    <Alert className="workbench-notice" showIcon type="warning" title={t("超时不等于失败，也不代表可以退款")} description={t("无证据时继续冻结；损失豁免必须由两位独立财务审核员确认。")} />
    ...
    <Drawer title={t("待核查请求")} size={760} open={Boolean(selected)} destroyOnHidden onClose={() => setSelected(undefined)}>
      ...
      ]} /><BillingReviews key={`${projectID}/${selected.id}`} projectID={projectID} reservationID={selected.id} /></>}
    </Drawer>
  </>;
}
